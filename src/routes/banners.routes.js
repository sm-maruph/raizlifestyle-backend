...
router.get("/all", authenticate, requireAdmin, asyncHandler(async (_req, res) => {
  const { data, error } = await supabaseAdmin.from("banners").select("*").order("position");
  if (error) throw error;
  res.json({ items: data });
}));
...
router.post("/", authenticate, requireAdmin, upload.single("image"), asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    const [up] = await processMany("banners", [req.file], { folder: "banners", thumb: false });
...
router.put("/:id", authenticate, requireAdmin, upload.single("image"), asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin.from("banners").update(body).eq("id", req.params.id).select().single();
  if (error) throw error;
  res.json(data);
}));
